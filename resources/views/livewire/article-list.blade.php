<div id="post" class="w-full md:w-2/3 lg:w-3/4">
    @if ($search)
        <h3 class="text-lg mb-6 md:inline-flex md:items-center">
            <div class="inline-flex items-center font-semibold">
                <svg class="w-4 h-4 text-danger" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none"
                    viewBox="0 0 20 20">
                    <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="m19 19-4-4m0-7A7 7 0 1 1 1 8a7 7 0 0 1 14 0Z" />
                </svg>
                <p class="ml-2 md:mr-2">Pencarian: </p>
            </div>
            <p>{{ $search }}</p>
        </h3>
    @else
    ...
    @if ($this->articles->count())
        ...
        @endforeach
    @else
        <p class="text-lg"><span class="text-danger-300">*</span> Saat ini sedang tidak ada artikel</p>
    @endif

    <nav class="mt-8 lg:mr-4" aria-label="Article">
        {{ $this->articles->onEachSide(0)->links('vendor.pagination.tailwind') }}
    </nav>
</div>
